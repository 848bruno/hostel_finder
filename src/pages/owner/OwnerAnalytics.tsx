...
interface BookingDoc {
  _id: string;
  hostel: { _id?: string; name: string } | string;
  amount: number;
...
export function OwnerAnalytics() {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      const averageOccupancy = hostelStats.totalRooms > 0
        ? ((hostelStats.totalRooms - hostelStats.availableRooms) / hostelStats.totalRooms) * 100
        : 0;

      // Top hostel by bookings
      const hostelCounts: Record<string, { name: string; count: number }> = {};
      bookings.forEach(b => {
        const id = typeof b.hostel === 'string' ? b.hostel : (b.hostel as { _id?: string; name: string })?._id ?? 'unknown';
        const name = typeof b.hostel === 'string' ? id : (b.hostel as { name: string }).name;
        hostelCounts[id] = hostelCounts[id] ?? { name, count: 0 };
        hostelCounts[id].count++;
      });
      ...
      setAnalytics({
        ...
        activeBookings,
        averageOccupancy,
        topHostel: { name: top.name, bookings: top.count },
      });
    } catch (error) {
