...
export function StudentDashboard() {
  ...
  const [bookings, setBookings] = useState<(Booking & { hostel: Hostel })[]>([]);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (bookingsData) {
        setBookings(bookingsData as (Booking & { hostel: Hostel })[]);

        const active = bookingsData.filter(b => b.status === 'confirmed').length;
        const upcoming = bookingsData.filter(b =>
          b.status === 'pending' || (b.status === 'confirmed' && new Date(b.start_date) > new Date())
        ).length;
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {bookings.map((booking) => (
                  <div key={booking.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:border-blue-300 transition-colors">
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900">{booking.hostel.name}</h3>
                      <p className="text-sm text-gray-600 mt-1">{booking.hostel.address}</p>
                      <div className="flex items-center gap-4 mt-2 text-sm text-gray-500">
                        <span>{new Date(booking.start_date).toLocaleDateString()} - {new Date(booking.end_date).toLocaleDateString()}</span>
                        <span>KSh {Number(booking.total_amount).toLocaleString()}</span>
                      </div>
                    </div>
